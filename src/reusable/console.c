...
#include "usbd_cdc.h" //for USBD_CDC_SetRxBuffer();
...
void ConsoleInit(void)
{
	...
	USBD_CDC_SetRxBuffer(&hUsbDeviceFS, (uint8_t*)&mReceiveBuffer);
...
eCommandResult_T LEDModeParam(const char * buffer, const uint8_t parameterNumber, ledMode* parameterLED)
{
			...
			if ( COMMAND_SUCCESS == result )
			{
				...
				if (strcmp(str, "depth") == 0)
					*parameterLED = depth;
				else if (strcmp(str, "rate") == 0)
					*parameterLED = rate;
				else if (strcmp(str, "shape") == 0)
					*parameterLED = shape;
